...
###################################################################################################
error_align = open(pathFiltered + "_noalignment.txt", "a")

with open(IDList, 'r') as f:
    for ID in f.readlines():
        ID = ID.strip("\n")
        AlignFile = PathAlign + "/" + ID + ".aln.best_hmm.fasta.fsa"

        if os.path.exists(AlignFile):  # HmmCleaner can return empty alignment
            align = AlignIO.read(AlignFile, "fasta")
            align_length = align.get_alignment_length()

            if align_length > 0:  # Gblocks can return empty alignment
