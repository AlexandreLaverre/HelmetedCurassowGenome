...
sub printHelp{
    ...
    print "\n";
    print "This script selects transcripts that can be added to existing annotations.\n";
    print "\n";
...
my @defaultpars=("pathAnnot1", "pathAnnot2", "chrList",  "pathOutputDecision");
...
readGTF($parameters{"pathAnnot1"}, \%chromo, \%exoncoordsens, \%exontxens, \%genetxens, \%txgeneens, \%txexonens);
...
readGTF($parameters{"pathAnnot2"},  \%chromo, \%exoncoordsass, \%exontxass, \%genetxass, \%txgeneass, \%txexonass);
...
print $output "TranscriptID\tGeneID\tDecision\tReason\tOverlappingReferenceGenes\n";
...
foreach my $gene (keys %genetxass){
    ...
    if($nbag==0){
	## new gene, there is no overlap with anything in the reference annotation
	foreach my $tx (keys %{$genetxass{$gene}}){
