...
if [ -e ${pathResults}/diamond_results/GeMoMa_vs_${refsp}.diamond.blastp.out ]; then
    ...
    echo "diamond blastp --db ${pathProteinSequences}/${prefix} --query ${pathResults}/filtered_predictions_formatted.faa --out ${pathResults}/diamond_results/GeMoMa_vs_${refsp}.diamond.blastp.out --evalue 0.001 --outfmt 6 qseqid sseqid pident length mismatch gapopen qstart qend sstart send evalue bitscore gaps " >> ${pathScripts}/bsub_script_diamond
