...
if [ ${restore} = "false" ]; then
    ...
    ## restore
    docker run -v ${path}:/ifb/data/mydatalocal/HelmetedCurassowGenome --rm -t quay.io/comparative-genomics-toolkit/cactus:v1.3.0 cactus  --restart --binariesMode local --workDir ${pathResults}/ --maxServiceJobs 1 --maxCores ${nthreads} --maxMemory 120G ${pathResults}/jobStore ${pathResults}/seqFile ${pathResults}/alignment.hal
fi
