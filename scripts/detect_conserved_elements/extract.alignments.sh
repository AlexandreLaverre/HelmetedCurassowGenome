...
if [ ${dataset} = "no_protuberance" ]; then
    export targetGenomes="Struthio_camelus_australis,Dromaius_novaehollandiae,Gallus_gallus,Meleagris_gallopavo,Penelope_pileata,Alectura_lathami,Anas_platyrhynchos_platyrhynchos,Grus_americana,Calidris_pugnax,Upupa_epops,Rhinopomastus_cyanomelas,Strix_occidentalis_caurina,Ficedula_albicollis,Aquila_chrysaetos_chrysaetos,Geospiza_fortis,Parus_major,Serinus_canaria,Strigops_habroptila,Coturnix_japonica"
fi
...
if [ ${chr} = "all" ]; then
    ...
    docker run -v ${path}:/ifb/data/mydatalocal/IPLOSS --rm -t quay.io/comparative-genomics-toolkit/cactus:v1.3.0 hal2mafMP.py ${pathHAL}/366-avian.hal ${pathResults}/aln.maf  --targetGenomes ${targetGenomes} --refGenome Anas_platyrhynchos_platyrhynchos --refSequence ${chr} --start=${start} --length=${length}  --noDupes 
fi
